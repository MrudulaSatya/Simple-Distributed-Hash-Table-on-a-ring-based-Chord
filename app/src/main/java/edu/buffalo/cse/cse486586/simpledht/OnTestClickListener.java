...
public class OnTestClickListener implements OnClickListener {
	...
	private ContentValues[] initTestValues() {
		...
		for (int i = 0; i < TEST_CNT; i++) {
			...
			cv[i].put(VALUE_FIELD, Integer.toString(i));
			Log.e("added values",cv[i].getAsString("key")+" "+cv[i].getAsString("value"));
		}
	...
	private class Task extends AsyncTask<Void, String, Void> {
		...
		private boolean testInsert() {
			try {
				for (int i = 0; i < TEST_CNT; i++) {
					Log.e("writing content values",mContentValues[i].getAsString("key")+mContentValues[i].getAsString("value"));
					mContentResolver.insert(mUri, mContentValues[i]);
				}
			} catch (Exception e) {
				Log.e("caught exception writing content values",mContentValues[0].getAsString("key")+mContentValues[0].getAsString("value"));
